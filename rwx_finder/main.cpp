...
auto main() -> int
{
	...
	if ( std::filesystem::is_directory( start_path ) ) {
		...
		auto dll_cache = std::vector< std::filesystem::directory_entry >();
		for ( auto& p : std::filesystem::recursive_directory_iterator( start_path, std::filesystem::directory_options::skip_permission_denied ) ) {
			...
			dll_cache.push_back( p );
		}

		if ( !dll_cache.empty() ) {
			std::cout << "scanning " << dll_cache.size() << " dll's..." << std::endl << std::endl;

			std::for_each( std::execution::par_unseq, dll_cache.begin(), dll_cache.end(), [](std::filesystem::directory_entry& p ) {
				...
				if ( file_mapping == nullptr || !file_mapping.is_valid() ) {
					return;
				}
			...
			std::cout << "could not find any dll" << std::endl;
		}
	}
	else {
